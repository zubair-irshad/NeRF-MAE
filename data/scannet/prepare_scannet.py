...
def prepare_scannet_scene(
    scene_path, output_path, num_train_samples=100, num_val_samples=20
):
    ...
    intrinsic_dir = os.path.join(scene_path, "intrinsic")
    ...
    if len(img_idxs) < 500:
        print(f"Warning: {scene_path} has less than 500 valid images")
        return
    ...
    print(
        f"{scene_name}: {len(sampled_idxs)} train samples, {len(sampled_val_idxs)} val samples"
    )

    train_df = pandas.DataFrame(
        [str(i) + ".jpg" for i in sampled_idxs], columns=["image"]
    )
    val_df = pandas.DataFrame(
        [str(i) + ".jpg" for i in sampled_val_idxs], columns=["image"]
    )

    train_df.to_csv(
        os.path.join(output_path, "train_set.csv"), index=False, header=False
    )
...
def prepare_scenes(
    scannet_path, output_path, num_scenes, num_train_samples, num_val_samples
):
    os.makedirs(output_path, exist_ok=True)

    scene_paths = os.listdir(scannet_path)
    scene_paths = [
        x for x in scene_paths if os.path.isdir(os.path.join(scannet_path, x))
    ]
    ...
    print("len(scene_paths): ", len(scene_paths))
    for scene_path in scene_paths:
        ...
        if (
            ...
            or not os.path.exists(intrinsic_dir)
            or not os.path.exists(pose_dir)
        ):
    ...
    print("len(valid_scenes): ", len(valid_scenes))
    print("valid_scenes: ", valid_scenes)
    fn = partial(
...
def copy_images(output_path, scene_path):
    ...
    for img in img_train:
        shutil.copyfile(
            os.path.join(img_train_src_dir, img), os.path.join(img_all_dir, img)
        )
        shutil.copyfile(
            os.path.join(img_train_src_dir, img), os.path.join(img_train_dir, img)
        )

    for img in img_test:
        shutil.copyfile(
            os.path.join(img_test_src_dir, img), os.path.join(img_all_dir, img)
        )
...
def copy_colmap_recon(recon_dir, scene_dir):
    ...
    shutil.copytree(
        sparse_dir, os.path.join(scene_dir, "colmap", "sparse"), dirs_exist_ok=True
    )
    shutil.copytree(
        sparse_train_dir,
        os.path.join(scene_dir, "colmap", "sparse_train"),
        dirs_exist_ok=True,
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--ddp_nerf_dir", type=str, help="Dense Depth Priors NeRF repo root."
    )

    parser.add_argument(
        "--num_scenes", type=int, default=120, help="Number of scenes to process."
    )
    parser.add_argument(
        "--num_train_samples",
        type=int,
    ...
    parser.add_argument(
        "--gpu", type=str, default="", help="GPU to use for COLMAP reconstruction."
    )
...
if __name__ == "__main__":
    ...
    scans_dir = os.path.join(args.scannet_dir, "scans_test")
    ...
    print("====================================\n\n")
    print("Select training and validation views\n")
    prepare_scenes(
        scans_dir,
        nerf_dir,
        args.num_scenes,
    ...
    print("====================================\n\n")
    print("Extract and copy RGB images\n")
    ...
    for scene in tqdm(scenes):
        extract_scene_rgb(
            os.path.join(
                args.ddp_nerf_dir, "preprocessing/build/extract_scannet_scene"
            ),
            args.scannet_dir,
    ...
    print("====================================\n\n")
    print("Copy COLMAP reconstruction to NeRF directory for training \n")
    # Copy COLMAP reconstruction to NeRF directory for training
    for scene in tqdm(scenes):
        copy_colmap_recon(
            os.path.join(colmap_recon_dir, scene, "recon"),
            os.path.join(nerf_dir, scene),
        )
    ...
    print("====================================\n\n")
    print("Create poses with rgb_only = False now \n")
    # Extract and copy RGB images
    scenes = os.listdir(nerf_dir)
    for scene in tqdm(scenes):
        extract_scene_rgb(
