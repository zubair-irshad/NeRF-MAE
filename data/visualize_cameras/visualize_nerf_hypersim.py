...
def visualize_cameras(colored_camera_dicts, sphere_radius, focal, bounding_boxes, camera_size=0.1, geometry_file=None, geometry_type='mesh', folder=None):
    ...
    for i in range(len(poses_dict)):
        ...
        all_c2w.append(C2W)
        img_size = (640, 480)
        frustums.append(get_camera_frustum(img_size, focal, C2W, frustum_length=2.0, color=color))
    for C2W in all_c2w:
        fig.plot_transform(A2B=C2W, s=2.0, strict_check=False)
...
if __name__ == '__main__':
    ...
    sphere_radius = 1.
    camera_size = 0.1
    colored_camera_dicts = {'train': ([0, 1, 0], transform_matrices)}
    # geometry_file = os.path.join(base_dir, 'mesh_norm.ply')
    # geometry_type = 'mesh'

    visualize_cameras(colored_camera_dicts, sphere_radius, focal_length,bounding_boxes,
