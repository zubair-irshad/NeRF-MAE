...
from nerf_rpn.model.metrics import psnr, mse
...
class Trainer:
    ...
    def build_model(self, args):
        ...
        self.model = SwinTransformer_MAE3D(
            patch_size=[4, 4, 4],
        ...
        if self.args.dataset != "general":
            with np.load(self.args.dataset_split) as split:
                ...
                if self.args.train_all:
                    self.train_scenes = np.concatenate(
    ...
    def train_loop(self):
        ...
        # max_lr = self.args.lr
        # min_lr = 1e-6  # The minimum learning rate you want to reach
        ...
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                (
                    ...
                    # mask_patches,
                    target,
                ) = self.model(rgbsigma, is_eval=True)
                ...
                mask = mask.bool()
                mse_value = mse(pred[..., :3], target[..., :3], valid_mask=mask)
                psnr_value = psnr(pred[..., :3], target[..., :3], valid_mask=mask)

            psnr_list.append(psnr_value.item())
            all_mse.append(mse_value.item())

            # print(f"\n Eval MSE/PSNR:")
            # print(f"PSNR: {psnr_value}, MSR: {mse_value}")

            if self.args.wandb:
                wandb.log(
                    {
                        f"Val PSNR": psnr_value,
                        f"Val MSE": mse_value,
                        f"val_loss": loss.item(),
                        f"val_loss_rgb": loss_rgb.item(),
                    },
                    commit=False,
                )
