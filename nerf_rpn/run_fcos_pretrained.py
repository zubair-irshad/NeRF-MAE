...
from model.feature_extractor import Bottleneck, ResNet_FPN_256
...
from model.feature_extractor import (
    SwinTransformer_FPN_Pretrained_Skip as SwinTransformer_FPN_Pretrained,
)

# from model.feature_extractor import (
#     SwinTransformer_FPN_Pretrained as SwinTransformer_FPN_Pretrained,
# )
...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        if args.checkpoint:
            ...
            self.logger.info(
                f"Loading checkpoint from previous checkpoint (Not MAE checkpoint):{args.checkpoint}."
            )
        ...
        if args.wandb and rank == 0:
            wandb.watch(self.model, log_freq=30)
        ...
        ####
        ### Uncomment here if you want to print MODEL
        ###
        # else:
        # self.logger.info(self.model)

        # Create a list to store the parameters that did not receive gradients

        # parameters = self.model.parameters()
        # parameters_without_grad = []

        # # Iterate over the parameters and identify the ones with no gradient
        # for i, param in enumerate(parameters):
        #     if param.grad is None:
        #         parameters_without_grad.append((i, param))

        # Print the layers or parameters that did not contribute to the loss function
        # self.logger.info("=============================================\n")
        # for index, param in parameters_without_grad:
        #     self.logger.info(f"Index: {index}, Parameter: {param}")
    ...
    # Define a function to extract and sort the metric value based on metric_name
    def extract_and_sort_metric(self, filename, metric_name):
        parts = filename.split("_")
        if len(parts) >= 2:
            metric_part = parts[-1].replace(".pt", "")
            if metric_name == "ap25":
                return float(metric_part.split("_")[0])
            elif metric_name == "ap50":
                return float(parts[-2])
                # return float(metric_part.split('_')[1])
        return 0.0  # Default value if the filename doesn't match the expected format

    def delete_suboptimal_checkpoints(
        ...
        # Determine the number of checkpoint files to keep
        num_files_to_keep = min(max(max_to_keep, min_to_keep), len(checkpoint_files))
        # Determine the checkpoint filenames to keep
        checkpoints_to_keep = checkpoint_files[-num_files_to_keep:]
        # Delete checkpoints that are not in the top N
    ...
    def train_loop(self):
        ...
        for epoch in range(1, self.args.num_epochs + 1):
            ...
            if epoch % self.args.eval_interval == 0 or epoch == self.args.num_epochs:
                ...
                metric_ap50 = APs["ap50"]
                metric_ap25 = APs["ap25"]
                # metric = recalls[-1]
                ...
                if self.best_metric_ap25 is None or metric_ap25 > self.best_metric_ap25:
                    self.best_metric_ap25 = metric_ap25
                    self.save_checkpoint(
                        epoch,
                        os.path.join(
                            self.args.save_path,
                            "model_best_ap25_ap50_"
                            + str(metric_ap50)
                            + "_"
                            + str(metric_ap25)
                            + ".pt",
                    ...
                    #     epoch, os.path.join(self.args.save_path, "model_best_ap50.pt")
                    # )
                # metric = APs[-1]
                ...
                # )
                self.delete_suboptimal_checkpoints(self.args.save_path, "ap50", 6, 1)
                # self.delete_suboptimal_checkpoints(self.args.save_path, "ap25", 3, 1)
    ...
    def eval(self, dataset):
        ...
        APs["ap50"] = ap50["ap"].item()
        APs["ap25"] = ap25["ap"].item()
...
def main_worker(proc, nprocs, args, gpu_ids, init_method):
    ...
    dist.init_process_group(
        backend="nccl",
        # init_method="tcp://127.0.0.1:17700",
