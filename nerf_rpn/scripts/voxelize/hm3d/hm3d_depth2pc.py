...
def depth2pc(depth_dir, mask_dir, scene_dir, points_per_obj=100000):
    ...
    # with open(os.path.join(scene_dir, "train", "transforms.json"), "r") as f:
    #     transforms_list = json.load(f)["frames"]

    # Sort the transforms list based on the file_path
    # json_file.sort(
    #     key=lambda x: int(os.path.splitext(os.path.basename(x["file_path"]))[0])
    # )

    with open(os.path.join(scene_dir, "train", "transforms.json"), "r") as f:
        transforms_file = json.load(f)["frames"]

    # Sort the transforms list based on the file_path
    transforms_file.sort(
        key=lambda x: int(os.path.splitext(os.path.basename(x["file_path"]))[0])
    )

    # print("transforms_file: ", transforms_file)
    ...
    print("len(depth_files): ", len(depth_files))
    ...
    for i in range(len(depth_files)):
        ...
        # Read depth
        with h5py.File(os.path.join(depth_dir, depth_files[i]), "r") as file:
            depth_img = np.array(file["data"])

        # frame = json_file["frames"][i]

        frame = transforms_file[i]

        # depth_img = h5py.File(os.path.join(depth_dir, depth_files[i]), "r")
        # depth_img = np.array(depth_img["depth"][:])

        with h5py.File(os.path.join(mask_dir, mask_files[i]), "r") as file:
            mask_img = np.array(file["data"])
...
if __name__ == "__main__":
    ...
    for s in tqdm(scenes):
        ...
        if os.path.exists(os.path.join(out_dir, s + ".npz")):
            print("Already done with scene: ", s, "pcd exists")
            continue
        ...
        pcd = depth2pc(
            ...
            os.path.join("/home/mirshad7/Downloads/masked_rdp_2_transforms", s),
        )
        write_npz(pcd, os.path.join(out_dir, s + ".npz"))

        print("Done with scene: ", s)
