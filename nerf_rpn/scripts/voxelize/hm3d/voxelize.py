...
if __name__ == "__main__":
    ...
    for s in tqdm(scenes):
        ...
        feat_dir = "/home/mirshad7/Downloads/hm3d_rpn_data/features"
        xform_dir = "/home/mirshad7/Downloads/masked_rdp_2_transforms"
        out_dir = os.path.join(dir, s, "voxel")
        out_dir_vis = os.path.join(dir, s, "voxel_vis")
        os.makedirs(out_dir, exist_ok=True)
        os.makedirs(out_dir_vis, exist_ok=True)

        scene_name = s.split(".")[0]
        if os.path.exists(os.path.join(out_dir, scene_name + ".npy")):
            print("Already done with scene: ", s, "voxel exists")
            continue

        pcd = np.load(os.path.join(pcd_dir, scene_name + ".npz"))
        points = pcd["points"]
        ids = pcd["ids"]
        ...
        write_ply(voxel, os.path.join(out_dir_vis, scene_name + ".ply"))
